...
def openai_chat_completions(
    ...
    *,
    ...
    ``use_json_object_format``: ``response_format: {type: json_object}`` —
    отключите через ``TRADENEWS_OPENAI_JSON_OBJECT=0``, если модель/прокси не поддерживает.
    ...
    want_json = use_json_object_format and (os.environ.get("TRADENEWS_OPENAI_JSON_OBJECT") or "1").strip().lower() in (
        "1",
        "true",
        "yes",
    )
    ...
    try:
        ...
        raise RuntimeError(f"OpenAI HTTP {e.code}: {err_body[:4000]}") from e
